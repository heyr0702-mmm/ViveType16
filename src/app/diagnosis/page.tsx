...
export default function Diagnosis() {
    ...
    const finishDiagnosis = (finalAnswers: Answers) => {
        // Encode answers to query string
        // Format: id:val,id:val...
        // To keep it short, we assume order is 1-28, so just val,val,val...
        // But to be safe, let's use a simple JSON string encoded
    ...
    // Preload next question image if we had any, but we don't.

    return (
        <NotebookLayout className="flex flex-col justify-between max-w-2xl mx-auto">
            {/* Header / Progress */}
            <div className="space-y-2">
                <div className="flex justify-between items-end border-b-2 border-ink pb-2">
                    <span className="font-bold font-handwriting text-xl">
                        Q.{currentQuestion.id} <span className="text-sm text-gray-500">/ {QUESTIONS.length}</span>
                    </span>
            ...
            <div className="flex-grow flex items-center justify-center py-8 sm:py-12 min-h-[200px]">
                <h2 className="text-xl sm:text-2xl font-bold text-center leading-relaxed animate-in fade-in slide-in-from-bottom-4 duration-500 key={currentQuestion.id}">
